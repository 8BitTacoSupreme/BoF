...
Plans:
- [ ] 03-01-PLAN.md -- Docker Compose infrastructure (cp-kafka, cp-schema-registry, cp-flink, sql-gateway) + Avro schemas + 3 data producers (retail, FSI, fraud) + kafkajs install
- [ ] 03-02-PLAN.md -- Backend services: flinkService.js (SQL Gateway REST client, session management, DDL/DML submission, job tracking) + kafkaConsumerService.js (per-request message tailing)
